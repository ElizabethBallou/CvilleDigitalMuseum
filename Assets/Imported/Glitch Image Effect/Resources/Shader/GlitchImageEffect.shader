Shader "Hidden/GlitchImageEffect"
{
    ...
    {
        float2 uvs = i.texcoordStereo.xy * _ScreenParams.xy;
        float4 c = 0;
        ...
        c = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, float2(xpos, uv.y));
        ...
        c.g = lerp(c.r, SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, float2(xpos + noise * 0.05, uv.y)).g, _Colored);
        c.b = lerp(c.r, SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, float2(xpos - noise * 0.05, uv.y)).b, _Colored);
        half4 mainC = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv);
    ...
    float bnie(float2 uv, float threshold, float scale, float seed)
    {
        ...
        float noise2 = SAMPLE_TEXTURE2D(_NoiseTex, sampler_NoiseTex, noiseUV).r;
    ...
    half4 Frag2(VaryingsDefault i) : SV_TARGET
    {
        ...
        half4 mainC = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.texcoordStereo.xy);
    ...
    half4 Frag3(VaryingsDefault i) : SV_TARGET
    {
        float2 uv = i.texcoordStereo.xy;
        float4 c = 0;
        half4 mainC = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv);
        ...
        for( int i=0; i<NUM_SAMPLES; ++i )
        {
            ...
            uv.x = sat( uv.x + ofs * t );
            float4 samplecol = SAMPLE_TEXTURE2D( _MainTex, sampler_MainTex, uv);
    ...
    half4 Frag4(VaryingsDefault i) : SV_TARGET
    {
        float2 fc = i.texcoordStereo.xy;
        float4 c = 0;
        float2 uv = fc.xy;
        ...
        half4 mainC = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, fc.xy);
