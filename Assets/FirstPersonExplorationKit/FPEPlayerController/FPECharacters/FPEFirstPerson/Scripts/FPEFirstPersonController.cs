...
namespace Whilefun.FPEKit {
    ...
    public class FPEFirstPersonController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            nextStepInCycle = cumulativeStepCycleCount / 2.0f;

            m_Jumping = false;
            m_AudioSource = GetComponent<AudioSource>();
            gameObject.GetComponent<FPEMouseLook>().Init(transform, m_Camera.transform);
        ...
        private void ProgressStepCycle(float speed)
        {
            ...
            if (m_CharacterController.velocity.sqrMagnitude > 0.0f && (m_Input.x != 0.0f || m_Input.y != 0.0f))
            {
                ...
                if (isCrouching)
                {
                    cumulativeStepCycleCount += (m_CharacterController.velocity.magnitude + (speed * (isWalking ? walkStepLengthCrouching : runStepLengthCrouching))) * Time.fixedDeltaTime;
                }
                else
                {
                    cumulativeStepCycleCount += (m_CharacterController.velocity.magnitude + (speed * (isWalking ? walkStepLengthStanding : runStepLengthStanding))) * Time.fixedDeltaTime;
                }
        ...
        private void GetInput(out float speed)
        {
            ...
            float horizontal = FPEInputManager.Instance.GetAxis(FPEInputManager.eFPEInput.FPE_INPUT_HORIZONTAL);
            float vertical = FPEInputManager.Instance.GetAxis(FPEInputManager.eFPEInput.FPE_INPUT_VERTICAL);

            // Keep track of whether or not the character is walking or running
            isWalking = (runEnabled ? (!FPEInputManager.Instance.GetButton(FPEInputManager.eFPEInput.FPE_INPUT_RUN)) : true);
